...
class MatchupStats(Stats):
    ...
    def get_high_scoring_margin(league, week=None, top_n=1):
        ...
        if week is not None:
            head_to_head = league.get_head_to_head()
...
class PlayerStats(Stats):
    ...
    def get_high_player_score(league, week=None, top_n=1):
        ...
        """
        if week is None:
            eligible_player_scores = league.filter_out_optouts(
                league.player_scores, league
            )
        else:
            eligible_player_scores = league.filter_out_optouts(
                [score for score in league.player_scores if score["week"] == week],
                league,
            )
    ...
    def get_regular_season_high_scoring_player(league, top_n=1):
        ...
        player_totals = {}
        player_info = {}
        for player in league.player_scores:
